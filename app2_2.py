...
def obtener_recomendaciones_gemini(future_predictions_df, metrics_data):
    ...
    Args:
        future_predictions_df (pd.DataFrame): DataFrame con las predicciones futuras (Fecha, P42_2_Predicho).
    ...
    try:
        ...
        if not future_predictions_df.empty:
            ...
            for index, row in future_predictions_df.iterrows():
                # Asegurarse de que el nombre de la columna sea 'P42_2_Predicho'
                prompt_text += f"Fecha: {row['Fecha']}, Precipitación Predicha: {row['P42_2_Predicho']:.2f} mm\n"
        else:
...
def index():
    ...
    monthly_avg_html = monthly_avg_df.to_frame(name='P42_2_Promedio').to_html(classes='table table-striped table-bordered')
...
def predecir_y_analizar_precipitacion_sarima(ruta_csv, anios_a_predecir=2, test_size_ratio=0.2):
    ...
    try:
        # --- MODIFICACIÓN CLAVE AQUÍ: Añadir na_values=[''] a pd.read_csv ---
        # Esto le dice a pandas que trate las cadenas vacías '' como NaN
        ...
        # El pd.to_numeric con errors='coerce' ya convierte valores no numéricos (incluido NaN) a NaN
        # Es bueno mantenerlo para otras posibles no-números.
        df['P42_2'] = pd.to_numeric(df['P42_2'], errors='coerce') 

        # Estas líneas se encargan de rellenar los NaN, ya sea los originales del CSV
        # o los generados por reindex (meses faltantes).
        df['P42_2'] = df['P42_2'].interpolate(method='linear')
        df['P42_2'] = df['P42_2'].fillna(df['P42_2'].mean()) 
        ...
        # Volver a rellenar si reindex introdujo más NaN (meses completamente ausentes)
        df['P42_2'] = df['P42_2'].interpolate(method='linear')
        df['P42_2'] = df['P42_2'].fillna(df['P42_2'].mean())
        ...
        plt.plot(y.index, y, label='Precipitación P42_2', color='blue')
        plt.title('Serie de Tiempo de Precipitación Mensual (P42_2)')
        plt.xlabel('Fecha')
        plt.ylabel('Precipitación P42_2 (mm)')
        plt.grid(True)
        plt.legend()
        ...
        # Nombre de archivo de gráfico actualizado para P42_2
        plt.savefig('static/serie_temporal_precipitacion_P42_2.png') 
        plt.close()
        print("Gráfico de la serie temporal original guardado como 'static/serie_temporal_precipitacion_P42_2.png'")
        ...
        plt.tight_layout()
        plt.savefig('static/precipitacion_mensual_promedio_barras_42_2.png') 
        plt.close()
        print("Gráfico de barras de precipitación promedio mensual guardado como 'static/precipitacion_mensual_promedio_barras_42_2.png'")
        ...
        df_para_csv = pd.DataFrame({
            ...
            # Asegurarse de que el nombre de la columna sea 'P42_2_Predicho'
            'P42_2_Predicho': forecast_future_mean.round(3).values,
            #'Lower_CI': forecast_future_ci.iloc[:, 0].round(3).values,
            #'Upper_CI': forecast_future_ci.iloc[:, 1].round(3).values
        })
        # Nombre de archivo de salida actualizado
        nombre_archivo_salida = f"Predicciones_Precipitacion_FuturaP42_2_{anios_a_predecir}anios.csv"
        ...
        plt.plot(train.index, train, label='Datos de Entrenamiento (P42_2)', color='blue')
        # Etiqueta corregida aquí de P4 a P42_2
        plt.plot(test.index, test, label='Datos Reales de Prueba (P42_2)', color='green')
        plt.plot(forecast_test_mean.index, forecast_test_mean, label='Predicción en Prueba (P42_2)', color='orange', linestyle='--')
        ...
        plt.plot(forecast_future_mean.index, forecast_future_mean, label='Predicción Futura (P42_2)', color='red')
        ...
        plt.ylim(y.min() - 10, y.max() + 10) 
        # Nombre de archivo de gráfico actualizado para P42_2
        plt.savefig('static/prediccion_precipitacion_sarima_P42_2.png') 
        plt.close()
        print("Gráfico de predicciones del modelo SARIMA guardado como 'static/prediccion_precipitacion_sarima_P42_2.png'")
