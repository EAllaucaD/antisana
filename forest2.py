...
RUTA_CSV = 'P43_33.csv' # ¡CAMBIO AQUÍ: Nombre del archivo CSV!
...
def load_and_preprocess_data(ruta_csv):
    try:
        ...
        # ¡CAMBIO AQUÍ: Referencia a la columna de datos!
        ...
        # ¡CAMBIO AQUÍ: Referencia a la columna de datos!
        df['P43_33'] = df['P43_33'].interpolate(method='linear')
        df['P43_33'] = df['P43_33'].fillna(df['P43_33'].mean()) # Rellena cualquier NaN restante con la media
        ...
        # ¡CAMBIO AQUÍ: Retornamos la serie P43_33!
        return df['P43_33'] 
...
# Se utilizan los hiperparámetros por defecto o los que se definieron manualmente
...
# ¡CAMBIO AQUÍ: Título del gráfico!
plt.title('Precipitación Mensual (P43_33): Datos Reales y Predicciones con Random Forest')
plt.xlabel('Fecha')
# ¡CAMBIO AQUÍ: Etiqueta del eje Y!
...
if not os.path.exists('static'):
    os.makedirs('static')
# ¡CAMBIO AQUÍ: Nombre del archivo de imagen!
plt.savefig('static/prediccion_precipitacion_P43_33_ml_test.png')
plt.close()
print("Gráfico de predicciones del modelo ML en conjunto de prueba guardado como 'static/prediccion_precipitacion_P43_33_ml_test.png'")
...
final_future_predictions_df = pd.DataFrame({
    ...
    # ¡CAMBIO AQUÍ: Nombre de la columna de predicción en el CSV de salida!
    'P43_33_Predicho': future_predictions_ml 
})

# --- Guardar en un CSV diferente para las predicciones (con el nuevo nombre P43_33) ---
# ¡CAMBIO AQUÍ: Nombre del archivo CSV de salida!
nombre_archivo_salida_ml_futuro = f"Predicciones_Precipitacion_Futura_ML_P43_33_{ANIOS_A_PREDECIR}anios.csv"
final_future_predictions_df.to_csv(nombre_archivo_salida_ml_futuro, index=False)
print(f"\nPredicciones futuras del modelo ML (Random Forest para P43_33) guardadas en '{nombre_archivo_salida_ml_futuro}'")

# --- Visualización de Predicciones Futuras ---
plt.figure(figsize=(16, 8))
# ¡CAMBIO AQUÍ: Etiqueta en el gráfico!
...
plt.plot(final_future_predictions_df['Fecha'], final_future_predictions_df['P43_33_Predicho'], 
         # ¡CAMBIO AQUÍ: Etiqueta en el gráfico!
         label=f'Predicción Futura ML (Random Forest - P43_33 - {ANIOS_A_PREDECIR} años)', color='red', linestyle='--')
# ¡CAMBIO AQUÍ: Título del gráfico!
plt.title(f'Precipitación Mensual (P43_33): Datos Históricos y Predicción Futura con Random Forest')
plt.xlabel('Fecha')
# ¡CAMBIO AQUÍ: Etiqueta del eje Y!
plt.ylabel('Precipitación P43_33 (mm)')
plt.legend()
...
plt.tight_layout()
# ¡CAMBIO AQUÍ: Nombre del archivo de imagen!
plt.savefig('static/prediccion_precipitacion_P43_33_ml_future.png')
plt.close()
print("Gráfico de predicciones futuras del modelo ML guardado como 'static/prediccion_precipitacion_P43_33_ml_future.png'")