...
def obtener_recomendaciones_gemini(future_predictions_df, metrics_data):
    ...
    Args:
        future_predictions_df (pd.DataFrame): DataFrame con las predicciones futuras (Fecha, P42_Predicho).
    ...
    try:
        ...
        if not future_predictions_df.empty:
            ...
            for index, row in future_predictions_df.iterrows():
                prompt_text += f"Fecha: {row['Fecha']}, Precipitación Predicha: {row['P42_Predicho']:.2f} mm\n"
        else:
...
def index():
    ...
    monthly_avg_html = monthly_avg_df.to_frame(name='P42_Promedio').to_html(classes='table table-striped table-bordered')
...
def predecir_y_analizar_precipitacion_sarima(ruta_csv, anios_a_predecir=2, test_size_ratio=0.2):
    ...
    try:
        ...
        df['P42'] = pd.to_numeric(df['P42'], errors='coerce') 

        df['P42'] = df['P42'].interpolate(method='linear')
        df['P42'] = df['P42'].fillna(df['P42'].mean()) 
        ...
        plt.plot(y.index, y, label='Precipitación P42', color='blue')
        plt.title('Serie de Tiempo de Precipitación Mensual (P42)')
        plt.xlabel('Fecha')
        plt.ylabel('Precipitación P42 (mm)')
        plt.grid(True)
        plt.legend()
        ...
        df_para_csv = pd.DataFrame({
            ...
            'P42_Predicho': forecast_future_mean.round(3).values,
            #'Lower_CI': forecast_future_ci.iloc[:, 0].round(3).values,
            #'Upper_CI': forecast_future_ci.iloc[:, 1].round(3).values
        })
        nombre_archivo_salida = f"Predicciones_Precipitacion_FuturaP42_{anios_a_predecir}anios.csv"
        ...
        plt.plot(train.index, train, label='Datos de Entrenamiento (P42)', color='blue')
        plt.plot(test.index, test, label='Datos Reales de Prueba (P42)', color='green')
        plt.plot(forecast_test_mean.index, forecast_test_mean, label='Predicción en Prueba (P42)', color='orange', linestyle='--')
        ...
        plt.plot(forecast_future_mean.index, forecast_future_mean, label='Predicción Futura (P42)', color='red')
        ...
        plt.tight_layout()
        plt.ylim(y.min() - 10, y.max() + 10) 
        plt.savefig('static/prediccion_precipitacion_sarima42.png') 
        plt.close()
        print("Gráfico de predicciones del modelo SARIMA guardado como 'static/prediccion_precipitacion_sarima42.png'")
